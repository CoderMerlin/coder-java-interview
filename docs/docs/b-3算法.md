...
上面像大家介绍了非常简单的数组在算法中遇到的问题，其实很多基础算法也都是作用在数组上的。很多非常好的面试算法题都是在基础算法题上做一些变更。接下来我们再来向大家介绍移到非常简单的基础算法题。
在`LeetCode`算法题中的第75道：https://leetcode-cn.com/problems/sort-colors/

![image-20201112225102599](https://cdn.jsdelivr.net/gh/CoderMerlin/blog-image/images/interview/java20201112225141.png)

当我们看到题目的时候，能想到好的排序算法就用，我们的目的就是解决问题。如果没有好的排序算法，我们可以继续使用暴力方法解题。解决问题总比没有解决强。

- 可以使用任何一种排序算法
- 没有使用上题目中给出的特殊条件

对于我们这个数组，只有0,1,2三种数字，因此我们只需要扫描一遍整个数组，然后分别统计出0,1,2有多少就行。

我们可以使用最简单的：计数排序算法。分别统计出0,1,2都有多少，然后填充进数组中。

![计数排序统计1](https://cdn.jsdelivr.net/gh/CoderMerlin/blog-image/images/interview/java20201112230029.png)



![计数排序统计2](https://cdn.jsdelivr.net/gh/CoderMerlin/blog-image/images/interview/java20201112230035.png)
...
public static void sortColors(int[] nums) {
    int[] count = new int[]{0,0,0}; //存放0,1,2的计数
    for (int i =0; i<nums.length;i++){
        //assert (count.length>0 && count.length < 3);
        count[nums[i]]++;
    }
    int index = 0;
...
这种算法有一点不好的地方是：目前只有3个元素，只需要写3个for循环，如果有100个元素呢？那么就需要写100个for循环。因此我们要问自己：有没有更优的解法？我们有没有可能只扫描数组一遍，便完成排序？

我们来看下**三路快排** 思路：

![image-20201112230258766](https://cdn.jsdelivr.net/gh/CoderMerlin/blog-image/images/interview/java20201112230527.png)

对于不知道三路快排思路的同学可以先看下这篇文章：https://blog.csdn.net/k_koris/article/details/80585979

 

![三路快排](E:\blog\coder-java-interview\docs\docs\b-3算法.assets\java20201118230629.gif)
...
最直接的思考：暴力揭发。双层遍历！时间复杂度O(n^2)，一时没有思路的时候，也是一个可行解。

暴力解法在`LeetCode`上不可行，会出现超时，这是由于数据规模的原因！面试可以尝试一下。



### 思路二

我们可以看到**暴力解法**没有充分利用原数组的性质——有序。
...
由于我们要在数组中找到两个索引，这两个索引的值等于target，所以这两个索引的位置一定是一左一右存在的。因此在初识的情况下，我选着两个索引，一个在最左边，我叫做i，一个索引在最右侧，我叫做j。

我们要找的值是nums[i]+nums[j] = target

但是通常情况下我们的值是nums[i]+nums[j] < target，这种情况的时候，我们进行i++; 

如果是num[i]+num[j] < target的时候，我们就j--;因为 nums[j--]的值 小于nums[j]

这都是利用我们数组的有序性。这个过程我们就能找到nums[i]+nums[j] = target
...
- 空字符串如何看？
- 字符的定义？（只有数字和字母？还是所有的ASCLL码？）
- 大小写问题？

练习题：LeetCode334，LeetCode345
...
暴力解：遍历所有的子数组[i...j]，计算其和sum,验证sum >= s，时间复杂度O(n^3)
...
比如我们看到一个连续子数组nums[i...j]从i到j，

如果当前的连续子数组的和还不到s的话，那么我们就j++, 变为nums[i...j++]。

如果我们的和还不到s的话，继续j++，直到某个时刻看到sum>s。

如果sum>s的时候，我们就i--，这样我们的子数组的长度就少一些

在缩小i的某一个过程中，sum < s了。此时我们的j又向前进 j++。

在整个过程中，我们有一个窗口从i到j，一直向前滑动来满足题意，这也就是滑动窗口的意思。
